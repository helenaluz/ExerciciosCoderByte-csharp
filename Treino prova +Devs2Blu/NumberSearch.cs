...
namespace Treino_prova__Devs2Blu
{
    internal class NumberSearch
    {
        static void Main(string[] args)
        {
            ...
            {
                if (Char.IsLetter(frase[i])) count++;
                if (Char.IsNumber(frase[i])) { numeros.Add(int.Parse(frase[i].ToString())); }

            }
            int countNum = numeros.Sum();
            
            int result = (int) Math.Round((countNum/ (double)count));
